...
function swarmLaunchFromRun(run: ISessionSwarmService['run']): ISessionSwarmService['launch'] {
  return ((args) => ({
...
describe('Agent tool description', () => {
  ...
  it('renders the tool set for each subagent type', () => {
    ...
    expect(description).toContain(
      'Tools: TeamSend, TeamStatus, TeamWait, TeamTaskReport, TeamReviewSubmit, AskUserQuestion, Bash, Read, ReadMediaFile, Glob, Grep, WebSearch, FetchURL',
    );
    expect(description).toContain(
      'Tools: Agent, AgentSwarm, TeamSend, TeamStatus, TeamWait, TeamTaskReport, TeamAnswerQuestion, AskUserQuestion, Bash',
    );
    expect(description).toContain('AgentSwarm, AgentProfileCreate, TeamSend');
  });
...
describe('AgentSwarmToolInputSchema', () => {
  ...
  it('accepts independently named profession and model choices per item', () => {
    expect(
      AgentSwarmToolInputSchema.safeParse({
        ...
        items: [
          {
            item: 'inspect renderer state',
            display_name: '界面侦察',
            subagent_type: 'explore',
          },
          {
            item: 'implement the fix',
            display_name: '构建专家',
            subagent_type: 'coder',
            model: 'primary',
          },
  ...
  it('rejects display names containing unsupported punctuation', () => {
    expect(
      AgentSwarmToolInputSchema.safeParse({
        description: 'Review files',
        prompt_template: 'Review {{item}}',
        items: [
          { item: 'src/a.ts', display_name: '界面 侦察', subagent_type: 'explore' },
          { item: 'src/b.ts', display_name: '构建专家', subagent_type: 'coder' },
        ],
      }).success,
    ).toBe(false);
  });

  it('omits provider-incompatible Unicode patterns from display name parameters', () => {
    const schema = toInputJsonSchema(AgentSwarmToolInputSchema) as {
      readonly properties: {
        readonly items: {
          readonly items: {
    ...
    const structuredItem = schema.properties.items.items.anyOf.find((item) => item.type === 'object');
    const displayName = structuredItem?.properties?.['display_name'];

    expect(displayName).toMatchObject({ type: 'string', minLength: 1, maxLength: 24 });
    expect(displayName).not.toHaveProperty('pattern');
  });
  ...
  it('exposes subagent_type, resume_agent_ids, and model parameters', () => {
    ...
    expect(properties['resume_agent_ids']?.description).toContain('reusable existing subagent');
    expect(properties['model']?.description).toContain('secondary model');
...
describe('AgentSwarm tool description', () => {
  ...
  it('states the enforced input requirements', () => {
    ...
    expect(description).toContain('Available profession profiles');
    expect(description).toContain('AgentProfileCreate');
  });

  it('shows profession introductions and task fit so the parent can choose a suitable agent', () => {
    ctx = createTestAgent();

    const description = agentSwarmDescription();

    expect(description).toContain('Introduction: General software engineering agent');
    expect(description).toContain('Best for: Use this agent for non-trivial software engineering work');
  });
...
describe('AgentSwarm tool execution contract', () => {
  ...
  it('runs item-based swarms through the session swarm service and renders XML results', async () => {
    ...
    const swarmService: ISessionSwarmService = {
      ...
      launch: swarmLaunchFromRun(runSwarm as ISessionSwarmService['run']),
...
describe('Agent tools', () => {
  ...
  describe('registered user tools', () => {
    ...
    it('routes registered user tools through tool.call request/response', async () => {
      ...
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        system: <system-prompt>
        tools: Agent, AgentProfileCreate, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Lookup, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TeamAnswerQuestion, TeamReviewSubmit, TeamSend, TeamStatus, TeamTaskReport, TeamWait, TodoList, UpdateGoal, Write
        messages:
      ...
      expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
        ...
        [wire] llm.request                 { "kind": "loop", "provider": "openai", "model": "mock-model", "modelAlias": "mock-model", "thinkingEffort": "off", "maxTokens": 1000000, "toolSelect": false, "systemPromptHash": "ec9c34379c88babbc468ef2f3e0e08cd2f422c8c4a910664fb8bb394d703a575", "toolsHash": "1cb15e516ff2519e3f958bb171bef2a7f929a151328c781f4162f068e8b228ac", "messageCount": 4, "turnStep": "0.2", "time": "<time>" }
    ...
    `);
      ...
      `);
      expect(ctx.lastLlmInput()).toMatchInlineSnapshot(`
        tools: Agent, AgentProfileCreate, AgentSwarm, AskUserQuestion, Bash, CreateGoal, Edit, EnterPlanMode, ExitPlanMode, FetchURL, GetGoal, Glob, Grep, Read, SetGoalBudget, Skill, TaskList, TaskOutput, TaskStop, TeamAnswerQuestion, TeamReviewSubmit, TeamSend, TeamStatus, TeamTaskReport, TeamWait, TodoList, UpdateGoal, Write
        messages:
          <last>
