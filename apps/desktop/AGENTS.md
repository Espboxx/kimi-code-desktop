...
- During release preparation, run the complete local gate:
  ...
  It performs the frozen install, Desktop tests, typecheck, license check, build, Electron E2E, Windows x64 portable packaging, packaged-resource/native-module inspection, packaged smoke, and SHA256 generation. Target-native macOS arm64 and Linux x64 packages are verified by GitHub Actions.
- After committing the unchanged verified tree on `main`, publish it with:
  ...
  `Publish` pushes `main`, creates and pushes the matching Desktop tag, waits for the Desktop Release workflow, and verifies the Windows portable EXE, macOS DMG, Linux AppImage/DEB, and every SHA256 asset from GitHub Release.
