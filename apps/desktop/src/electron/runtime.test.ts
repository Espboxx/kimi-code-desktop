  ...
  it('does not publish a stale workspace refresh after an A-to-B switch', async () => {
    ...
    const runtime = new KimiDesktopRuntime({
      ...
      host: {
        ...
        openExternal: async () => {},
        openPath: async () => {},
        getUpdateState: () => ({ currentVersion: '0.5.0', mode: 'manual', status: 'idle' }),
        checkForUpdates: async () => ({ currentVersion: '0.5.0', mode: 'manual', status: 'up-to-date' }),
        downloadUpdate: async () => ({ currentVersion: '0.5.0', mode: 'manual', status: 'downloaded' }),
        installUpdate: () => {},
