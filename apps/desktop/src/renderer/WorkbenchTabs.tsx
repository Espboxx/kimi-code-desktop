import { Bot, FileCode2, GitCompare, X } from 'lucide-react';
...
export function WorkbenchTabs({ state, sessions, statuses, pendingCounts, onActivate, onClose }: {
          ...
          return (
            ...
            >
              <button className="workbench-tab-main" onClick={() => onActivate(tab)} title={tabTitle(tab, session)}>
                {tab.kind === 'session' ? <Bot size={13} /> : tab.kind === 'file' ? <FileCode2 size={13} /> : <GitCompare size={13} />}
