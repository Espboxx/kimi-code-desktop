...
When [Team mode](../customization/agents.md#team-mode) is active, `AgentSwarm` instead accepts structured items with stable `task_key`, `depends_on`, `display_name`, per-task `subagent_type`, workspace access, validation, and model routing. It returns after the durable batch is accepted; the Team scheduler enforces the session policy and completes the graph in the background. Use the `/team` controls to inspect and manage that work.
