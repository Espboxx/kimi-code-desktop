...
When [Team mode](../customization/agents.md#team-mode) is active, ordinary `Agent` delegation is unavailable. Use `AgentSwarm` so that every child remains a durable Team assignment.
...
When [Team mode](../customization/agents.md#team-mode) is active, `AgentSwarm` instead accepts structured items with stable `task_key`, `depends_on`, `display_name`, per-task `subagent_type`, workspace access, validation, and model routing. It returns after the durable batch is accepted and ends the delegator's current turn; the Team scheduler enforces the session policy and completes the graph in the background. Each child completion automatically wakes the direct delegator with its result, so `TeamWait` is reserved for explicit one-off synchronization rather than routine completion waiting. Use the `/team` controls to inspect and manage that work.
...
In an active Team, a non-leader member's `AskUserQuestion` is sent to the Team leader through the Team channel and does not open a user interaction in the member's session. The leader answers through the Team channel; the normal user interaction is used only when the leader asks a question.
