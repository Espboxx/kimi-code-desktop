...
| `KIMI_CODE_EXPERIMENTAL_TEAM_COLLABORATION` | Enable or disable Team collaboration in the v2 engine. The capability is enabled by default, but each session still requires explicit activation with `/team start` | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
