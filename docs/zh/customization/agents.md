...
Team 模式是会话级的持久协调器，适合需要多个专业子 Agent、任务依赖、隔离代码改动和一次统一审阅集成的工作。该能力默认可用，但必须在当前会话中显式启动：
...
不覆盖策略时，Team 最多同时运行 4 个任务、保留 16 名成员、允许 2 层嵌套委派，并分别重试执行 1 次、验证 2 次。token 和时长预算均为可选项。达到已配置预算后，Kimi Code 会暂停新的调度和模型请求；已经在途的请求仍可能完成。

启动后，`AgentSwarm` 会变成持久、非阻塞的任务图启动器。每个新任务都有稳定的任务键、显示名称、Agent profile、工作区策略、验证策略和可选模型分配；`depends_on` 依赖会让任务等待前置任务完成。启动器接收批次后立即返回，Team 调度器在后台继续执行，并记录任务、尝试、产物、审阅、消息、预算用量和集成状态。

Team 模式处于启动状态时，所有新的委派都必须通过 `AgentSwarm` 完成；普通 `Agent` 调用会被拒绝，避免子任务脱离持久任务图。启动后，直接派发任务的 Agent 会结束当前轮次；其任一子任务结束时，系统都会携带结果自动唤醒该 Agent。`TeamWait` 仍可用于一次明确的同步事件，但无需仅为等待子任务结束而调用。

提问也沿用同一条协调链。Team 成员调用 `AskUserQuestion` 时，Kimi Code 会通过 Team 频道把结构化问题发送给 Team 组长，而不会在成员会话中打开用户交互。组长通过 Team 频道作答；只有组长自己提出的问题才会呈现给用户。

工作区处理取决于任务类型：

- 只读任务通过只读工具共享主工作区。
- 写任务要求工作区是 Git 仓库，并在隔离的 worktree 中运行。主 worktree 存在未提交改动时，Kimi Code 会拒绝准备或应用 Team 写任务。
- 每个写候选结果都由独立验证 Agent 在单独的验证 worktree 中检查；通过的候选结果会合并到统一的集成 worktree。
- 在预览汇总 Diff 并显式确认 `/team apply` 之前，主 worktree 不会改变。应用或丢弃后，本次 Team 结果即关闭；如需再次运行 Team，应新建会话。

使用 `/team` 或 `/team status` 查看调度器、任务图、预算和集成状态。你可以暂停或恢复调度，取消、重试或重新分配任务，发送广播或私信，查看产物，以及预览、应用或丢弃汇总结果。完整命令见[斜杠命令](../reference/slash-commands.md#team-模式)。

Team 状态会随会话持久化。恢复会话时，未完成工作会标记为中断，调度器保持暂停；检查状态后运行 `/team resume`。包含旧版 Team v1 日志的会话仍可查看，但只能读取，不会把新的 v2 操作混入旧日志。
