...
启用 [Team 模式](../customization/agents.md#team-模式)后，`AgentSwarm` 会改用结构化任务项，支持稳定的 `task_key`、`depends_on`、`display_name`、逐任务 `subagent_type`、工作区访问、验证和模型路由。持久批次被接收后工具立即返回，Team 调度器根据会话策略在后台完成任务图。使用 `/team` 命令查看和控制这些工作。
